...
function AddPickupAddress(props) {
  ...
  const { setPageLoader, config, isSeller, setIsSeller } = useCartContext();
  useEffect(() => {
    if (isSeller) {
      return history.push("/create-post");
    }
  }, [props, isSeller]);
  ...
  async function addAddress(e) {
    ...
    setPageLoader(true);
    try {
      await axios.post("/user/make-seller", input, config);
      customSuccessNotification("Pickup Address Added.");
      setIsSeller(true);
      if (props.checkout) {
        ...
        <form onSubmit={(e) => addAddress(e)}>
          <div class="col-12 mb-4">
            <div class="form-group mb-0">
              <h6 class="main-color">Add Pickup Address</h6>
            </div>
          </div>
...
export default withRouter(AddPickupAddress);
