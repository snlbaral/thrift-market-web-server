...
function IndexColor() {
  ...
  function distroy(e,id){
    ...
    return (
        ...
        <Link className="btn btn-secondary float-right mb-2" to='/admin/addcolor' > Add</Link>
    ...
    {colors.map(color=>{
      ...
      <tr key={color._id}>
        ...
        <td> {color.name}</td>
        <td>
        <Link className="btn btn-primary" to={`/admin/editcolor/${color._id}`}>Edit</Link>
        <button className="btn btn-danger" onClick={(e)=>distroy(e,color._id)} >Delete</button>
        </td>
