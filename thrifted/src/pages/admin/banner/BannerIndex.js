...
function BannerIndex() {
    ...
    useEffect(() => {
      ...
      axios.get('/banner',config).then(response=>{
    ...
    return (
        ...
        <Link className="btn btn-secondary float-right" to="/admin/addbanner">Add Banner</Link>
        ...
        <table className="table table-light bg-white table-striped border-0" id="myTable" cellPadding="5">
            ...
            {banners.map(banner=>{
              ...
              <tr key={banner._id}>
                ...
                <td>
                <Link className="btn btn-primary" to={`/admin/editbanner/${banner._id}`}>Edit</Link>
                <button className="btn btn-danger" onClick={()=>distroy(banner._id)} >Delete</button>
                </td>
              </tr>
              )})}
            </tbody>
